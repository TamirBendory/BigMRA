...
The cryo--EM problem is to estimate the structure of the molecule from the micrograph (or several micrographs). 
 All current methods in the field split the reconstruction procedure to several subroutines. The first step in all methods is the so called particle picking, detecting the 2D tomographic projections of the samples from the noisy micrograph. The output of ideal particle picker is a series of 2D images, each contains one centered tomographic projection associated with unknown 3D orientation. These series of images 
 is then used by a variety of algorithms to reconstruct the structure. 
However, due to the high noise level, the performance of the particle pickers  is not ideal. For instance, the projections in the 2D images are typically not centered, increasing dramatically the number of parameters involved in the estimation problem. In addition, the information of particles that are too close to each other is usually neglected. Hence, valuable information that can be used is omitted. 
...
In our model, the noisy data is the analog of the micrograph. The $K$ sought signals correspond to $K$ different molecule's projections, taken from different orientations in $SO(3)$. In tomography, if enough projections are estimated, then one can estimate the whole structure (up to some $K$-dependent resolution).
The position of individual projections are the nuisuance varialbles in the cryo--EM problem, in analogy to the individual signal's locations in our setup.   

Clearly, applying the autocorrelation method to the cryo--EM problem raises interesting challenges that are far beyond the scope of this paper. 
For instance, the numerical experiments of Section~\ref{sec:numerics} indicate that we need a huge number of observations to get accurate estimate. Can we accelerate the estimation rate so we can apply this method with smaller amount of data? In addition, since the size of a micrograph is quite large (typically around $4000\times 4000$ pixels), computing the third-order autocorrelations may be  expensive if not implemented carefully by parallelizing the computations.
Also, the model should incorporate minimal spacing between the samples on the micrograph.
