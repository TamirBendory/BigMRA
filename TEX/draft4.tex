...
Indeed, the aim is only to reconstruct the molecule: estimating the locations of the particles in the micrograph is merely a helpful intermediate stage when it can be done. Our main message is that, by bypassing particle picking, one can, in principle, \TODO{reconstruct arbitrarily small molecules.}

\TODO{Paragraph about the simplified model + moments approach + announce the results}
...
For simplicity, we ignore noise affects and assume that the projections are sufficiently separated.
The $(\Delta_x,\Delta_y)$ entry of the second-order autocorrelation of the micrograph is equal, up to a constant that depends on the number of projections, to
\begin{equation}
M_2(\Delta_x,\Delta_y) \propto \sum_{n=1}^N \sum_{x,y}P_n(x,y)P_n(x+\Delta_x,y+\Delta_y),
\end{equation}
where $P_n$ denotes the $n$th projection. By taking $n\to\infty$ and assuming uniform distribution of viewing directions, we get 
\begin{equation}
...
\begin{equation} 
M_3(\Delta_x^1,\Delta_y^1;\Delta_x^2,\Delta_y^2) \propto \sum_{x,y}\int_{\omega} P_\omega(x,y)P_\omega(x+\Delta_x^1,y+\Delta_y^1)P_\omega(x+\Delta_x^2,y+\Delta_y^2)d\omega.
\end{equation}

In order to determine the particle, by~\eqref{eq:volume} one needs to estimate an order of $L^3$ spherical harmonics coefficients.
If the pixel size is proportional to $1/L$ (to match the volume's resolution), then $M_3$ provides an order of $L^4$ equations of $P_\omega$ which depends (after coordinate transformation) linearly in the spherical harmonic coefficients through~\eqref{eq:P}. 
