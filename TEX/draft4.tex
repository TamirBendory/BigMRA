...
In each micrograph, we place Einstein's image of size $50\times 50$  in random locations, while preserving the separation condition~\eqref{eq:spacing}.  
...
In this experiment, we assume we know the noise level $\sigma$ as well as the total number of occurrences of the target image across all micrographs.
In start contrast with the 1-D setup, the second-order autocorrelation determines almost any target image uniquely, up to reflection through the origin~\cite{hayes1982reconstruction,bendory2017fourier}.\footnote{\TODO{Should we cite \cite{bendory2017fourier} here? I'm not against citing it somewhere, but for the specific claim being justified here, it seems the older reference should be enough.}} This is because the second-order autocorrelations correspond to the Fourier magnitudes of the signal through the 2-D Fourier transform. 
...
The map is designed so that, if the estimated power spectrum is exact, then fixed points contain Einstein's image in the upper-left corner of size $L \times L$, and zeros elsewhere. The operator $P_2(z)$ combines the Fourier phases of  the current estimation $z$ with the estimated Fourier magnitudes. The operator $P_1(z)$ zeros out all entries of $z$ outside the $L\times L$ upper-left corner. 
