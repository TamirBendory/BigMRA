...
The separation condition guarantees that if $\mb i=(i,j)$ is in the support of some projection, then $\mb i + \mb \Bell$ for $\Bell\in[0,P-1]^2$ is either in the support of the same projection or outside the support of any projection. %Formally, $\mb i$ is in the support of the $j$th projection if and only if $0\leq \mb i - \mb s_j < L$ where the inequalities apply to each coordinate. Then for $||\Bell||_{\infty}\leq L-1$ we have
...
%In particular, they are invariant under in-plane rotations of the projections, and can therefore be block-diagonalized in any steerable basis, a basis in which an in-plane rotation corresponds to a phase shift of the expansion coefficients - see \cite{bhamre2016denoising,landa2017steerable,zhao2016fast}. 
