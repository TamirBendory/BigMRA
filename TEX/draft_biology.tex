...
\end{equation*}
where the summation is for $\mb i $ ranging over the $N^2$  pixels of the micrograph. %, $\Delta \mb i = (\Delta i, \Delta j)$.% and we set $\II(i,j)=0$ if either $i,j\notin\{1,\ldots, M\}$. 
...
Under the specified conditions, we show in Appendix TKTK that the first three autocorrelations  of the micrographs are related to those of the projections by \TODO{To fix}
