...
Other problems solve for parameterized models of $x$; for instance, see \cite{andrieu2001bayesian}. This paper also considers multiple distinct signals $x$ embedded in the same observation vector $Y$, as in our heterogeneity framework. Their proposed solution is a Markov Chain Monte Carlo algorithm designed for their specific parametrized problem.

Because likelihood methods are computationally expensive, methods based on recovery from moments and cumulants, which are akin to our method, have also been previously used for system identification. The question of identifiablity of $x$ from third-order statistics with non-Gaussian $W$ was considered in \cite{lii1982deconvolution}. A method using fourth-order cumulants is described in \cite{tugnait1984id}, while a method using fourth-order cumulants is described in \cite{giannakis1989id}.


%%%The problem of system identification should be distinguished from the related problem of blind deconvolution. In the latter problem, the goal is to identify input signal $W$, rather than the filter $x$.
