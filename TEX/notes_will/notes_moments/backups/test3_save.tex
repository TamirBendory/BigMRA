...
Similarly, if $\Delta_1 = 0$, then there is a surviving cross-term which converges to $\gamma \cdot \sigma^2 \cdot \bar{x}$ as well. So in summary, for all $0 \le \Delta_1 \le \Delta_2 \le L-1$, we have:
%
\begin{align}
%
...
\section{Estimating $J$, the number of copies of $x$}

Using the moments calculation, we can estimate the number of copies of $x$ embedded in $Y$, which we call $J$. This permits us to then estimate the unscaled moments of $x$, which in turn lets us fit $x$ to its moments.

From the first moment we estimate $(J / n) (\mathbf{1}^\top x)$, while from the second moment we estimate $(J/n) (\mathbf{1}^\top x)^2$. We can then estimate $J$ via
%
\begin{align}
%
    J = n \cdot \frac{(J / n)^2 (\mathbf{1}^\top x)^2}{(J/n) (\mathbf{1}^\top x)^2}.
%
\end{align}
%
