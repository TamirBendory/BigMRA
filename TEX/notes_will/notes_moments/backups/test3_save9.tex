...
\section{A quadratic equation satisfied by $\sigma$}

It turns out that $\sigma^2$ satisfies a quadratic equation, using third-order moments. The previous section gives the equation:
%
\begin{align}
%
    Q = \frac{A}{B - \sigma^2}.
...
\begin{align}
%
    D \equiv Q\cdot w + (2L+1)\cdot\sigma^2\cdot (Q \cdot \1^\top x).
%
\end{align}
...
\begin{align}
%
    D\cdot Q - Q \cdot (2L+1)\cdot\sigma^2\cdot (Q \cdot \1^\top x)
    = Q^2 \cdot w = C - \sigma^2 \cdot (Q \cdot \1^\top x),
%
\end{align}
%
and therefore
...
\begin{align}
%
    Q = \frac{C - \sigma^2 \cdot (Q \cdot \1^\top x)}
            {D - (2L+1)\cdot\sigma^2\cdot (Q \cdot \1^\top x)}
%
\end{align}

On the other hand, from the first equation relating $Q$ and $\sigma^2$, we see that:
%
\begin{align}
%
    Q = \frac{A}{B - \sigma^2}.
...
%
    \frac{A}{B - \sigma^2}
    = \frac{C - \sigma^2 \cdot (Q \cdot \1^\top x)}
        {D - (2L+1)\cdot\sigma^2\cdot (Q \cdot \1^\top x)}
    = \frac{C - E \cdot \sigma^2}{D - F \cdot \sigma^2},
%
\end{align}
%
where
...
\begin{align}
%
    F = (2L+1) \cdot Q \cdot \1^\top x.
%
\end{align}

This is a quadratic equation for $\sigma^2$. Unfortunately, it can have two positive solutions, so I'm not sure if it helps...
\end{document}
