...
Because likelihood methods are computationally expensive, methods based on recovery from moments and cumulants, which are akin to our method, have also been previously used for system identification. The question of identifiablity of $x$ from third-order statistics with non-Gaussian $W$ was considered in \cite{lii1982deconvolution}. A method using third-order cumulants is described in \cite{giannakis1989id}, while a method using fourth-order cumulants is described in \cite{tugnait1984id}.
