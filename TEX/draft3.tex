...
All contemporary methods in the field split the reconstruction procedure into several subroutines. 
The first step in the pipeline is  known as \emph{particle picking}, in which one identifies the locations of the 2-D tomographic projections of the samples embedded within the noisy micrograph. The output of an ideal particle picker is a series of 2-D images, each containing one centered tomographic projection associated with an unknown viewing direction and an in-plane rotation. This series of images 
...
Nowadays, it is also still popular to pick particles manually. This method, while it exploits the researcher's experience, is both tedious and subject to model bias.

The significance of  model bias in cryo--EM was stressed in~\cite{shatsky2009method,henderson2013avoiding} and demonstrated by the ``Einstein from noise'' example.
In this experiment, the image of Einstein is correlated with multiple images of pure white noise. The noisy images are aligned to Einstein's image using cross-correlation and then averaged. 
In~\cite{shatsky2009method}, it was shown that with merely  1000 such noisy images, the averaged image is remarkably similar to the template image of Einstein rather than to pure noise.
...
This is true in general for any method which is based on template matching.
\TODO{TB: Do we have a nice experiment to exemplify the phenomenon?}
...
The left panel shows the data without noise. In the middle panel,  i.i.d.\ Gaussian noise with standard deviation $\sigma=0.5$  was added.  At this noise level, Einstein's locations are still easily identified. In this scenario it is likely that standard detection algorithms can locate the copies of the signal within the micrograph. In the right panel, we show the same data swamped in  
...
detection  of individual image occurrences is impossible---even if the true image is known---and therefore particle picking is impossible.  This phenomenon is explained in detail in Appendix~\ref{sec:theory}. \TODO{WL: I think this example should go in the previous section on particle picking. The main issue we're trying to overcome is when detection is impossible; the fact that we don't use a template seems to be an additional advantage of our method, but not the main selling point. So I think the impossibility of detection in sufficiently low SNR must be emphasized earlier.}
...
\begin{figure}[h!]
	...
	\begin{subfigure}[h]{0.33\textwidth}
		...
		\caption{$\sigma = 3$}
	\end{subfigure}
RR	\caption{\label{fig:micro_example} Example of data sequences (micrographs) of size $250\times 250$ with additive i.i.d.\ Gaussian noise with variance $\sigma^2$. Each micrograph contains four occurrences of a $50 \times 50$ image of Einstein.}	
\end{figure}
...
This method requires only one pass over the data and can recover the signals in any $\SNR$, if the signals appear enough times in the data. As a side note, we mention that expectation-maximization---a popular framework in SPR---is intractable for this problem (see Appendices~\ref{sec:related_literature} and~\ref{sec:theory}  for more details). 
...
Each micrograph contains multiple occurrences of Einstein's image  in random locations, while keeping a separation of at least $49$ pixels between the images in each direction. From the micrographs, we estimated the signal's power spectrum, which is equivalent to the second-order autocorrelation. Then, in order to estimate the signal itself, we applied a standard phase retrieval algorithm called relaxed-reflect-reflect (RRR)~\cite{elser2017matrix}.

As an initial guess, we picked an image of the physicist Issac Newton; see Figure~\ref{fig:newton}. If the algorithm was prone to model bias, we would expect to get as an output an image that resembles Newton, similar to the ``Einstein from noise'' effect. However, the experiment exhibits the desired phenomenon: the more data we collect, the better the reconstruction quality. 
...
\TODO{TB: Few comments: 1. We have a movie in the supplementary material. 2. Does the error also have the right slope? We'll know once we'll gather enough micrographs 3. Need to improve the micrograph's generation code  }
...
\begin{figure}[h!]
	...
	\begin{subfigure}[h]{0.3\textwidth}
		\centering
		\includegraphics[scale=0.35]{Newton}
		\caption{\label{fig:newton}Newton  (model)}
	\end{subfigure}%
	\begin{subfigure}[h]{0.3\textwidth}
		\centering
		\includegraphics[scale=0.35]{reconstruction1}
		\caption{\small $P = 512$}
	\end{subfigure}%
	\begin{subfigure}[h]{0.3\textwidth}
		\centering
		\includegraphics[scale=0.35]{reconstruction10}
		\caption{$P = 512\times 10$}
	\end{subfigure}%

	\begin{subfigure}[h]{0.3\textwidth}
		\centering
		\includegraphics[scale=0.35]{reconstruction100}
		\caption{$P = 512\times 100$}
	\end{subfigure}%
	\begin{subfigure}[h]{0.3\textwidth}
	\centering
	\includegraphics[scale=0.35]{reconstruction200}
	\caption{$P = 512\times 200$}
	\end{subfigure}%
	\begin{subfigure}[h]{0.3\textwidth}
	\centering
	\includegraphics[scale=0.35]{reconstruction840}
	\caption{$P = 512\times 840$}
	\end{subfigure}%

	\caption{\label{fig:Einst_example} Recovery of Einstein's image. The RRR algorithm is initialized with the image of the physicist Issac Newton in the upper-left panel. We show estimations of Einstein using $P$ number of micrographs, each containing $700$ image occurrences on average with an $\SNR$ of $1/20$. The signal's estimation RMSE is  $0.5874, 0.3731, 0.2517, 0.2137, 0.1534$. The power spectrum estimation RMSE is $0.0381, 0.012, 0.0038, 0.0027, 0.0013$. }
\end{figure}
...
\gamma_i = \frac{M_i L}{N},
\end{equation}
where $M_i$ is the number of times signal $x_i$  occurs, $L$ is the length of each signal and $N$ is the micrograph length.
These ratios $\gamma_i$ represent the {density} of the the signals in the data.
In the 1-D experiment, we do not assume knowledge of either the ratios $\gamma_i$ or the noise level $\sigma$.

In order to estimate the signals, we compute the first three autocorrelation functions of the data and then estimate the signals and their corresponding $\gamma_i$ using a nonconvex least-squares. 
As can be seen, given enough signal occurrences, we can estimate the signals accurately. 
...
\TODO{TB: 1. Figure~\ref{fig:1Dheterosignals} will be replaced  with a ``progress figure'' (like we did for Einstien) for each signals and a plot of the recovery error for all three signals. 2. We may want to replace the triangle signal}
...
\caption{\label{fig:error_per_micro} The left panel shows the RMSE of estimating the power spectrum of the signal from the data as a function of the number of  collected micrographs. As expected, the curve is linear with slope $-1/2$ in logarithmic scale. The right panel shows the RMSE  (in logarithmic scale) of recovering Einstein's image. \TODO{TB: The program continues running so these figures will be extended to the right (more micrographs).}}	
\end{figure}
...
\begin{figure}[t]
	...
	\caption{An experiment for estimating three one-dimensional signals simultaneously with noise level $\sigma=3$. Red signals are the ground truth (targets) and the blue signals are our estimations. Individual RMSE of the estimates: $0.0239393 / 0.208925 / 0.0335956$. The estimated $\gamma$'s are $0.02574 / 0.01693 / 0.00818$ and the true ones $0.02561 / 0.01707 / 0.00853$. The $\SNR$ is $1/12$. \TODO{TB: 1. We will replace the figure with ``progress'' plots 2. Need to replace the triangle signal 3. Need to add error curves}}
...
All current algorithmic pipelines for SPR using cryo--EM start with a particle picking stage to detect the 2-D tomographic projections of the samples in the micrograph.
The particle picking  algorithms tend to be prone to model bias and cannot work reliably on small particles in low $\SNR$ environment. 
Bypassing the particle picking stage and constructing a 3-D model directly from the data---without assuming prior knowledge about the particle to be estimated---can be used to reconstruct ab initio models to initialize a refinement algorithm.  Alternatively, it can be applied to generate templates for a particle picker which does not suffer from model bias.

In this paper, we examined a simplified model that, we believe, captures important features of the SPR problem. 
While the field is currently dominated by Bayesian methods such as EM, they are intractable for such problems. As an alternative, we propose to use an autocorrelation analysis technique. 
Methods based on autocorrelation analysis require only one pass over the data and therefore are usually computationally efficient. \TODO{TB: Do we want to mention Kam's method?}
%This property has been exploit before  for  ab inito modeling assuming ideal particle picking~\cite{kam1980reconstruction,levin20173d,singer2018mathematics}. 
That being said, the SPR model is far more complicated than the model presented here. In future research, we hope to bridge this gap.
...
This separation can be induced by careful experimental design \TODO{TB: ???}. 
If the signals are not separated, one can introduce additional parameters that model the distribution of the spacing  between signal occurrences and then compute explicitly the relation between the autocorrelation functions of the data and the signals. It is yet to be studied under what conditions on this distribution one can estimate the signals. 
...
Let $x_1,\ldots,x_K\in\RL$ be the target signals and let $y\in\RN$ be the observed data. 
...
\begin{align}
	...
	\label{eq:ac_general}
\end{align}
where the values of $z$ outside the indices $0, \ldots, L-1$ are taken to be zero.
...
For the purpose of the analysis, we consider  the asymptotic regime where $M$ grows proportionally to $N$. 
Specifically, we define the ratio of the measurement occupied by  the signal as
\begin{equation}
\gamma = \frac{M L}{N}.
\end{equation}
\TODO{WL: $\gamma$ should really be the limit of this ratio as $N$ grows to infinity; for any finite value of $N$ this ratio may be different. TB: The accurate definition is given in~\eqref{eq:gamma_limit}. We may want to leave it as is here.}
...
As before, we consider  the asymptomatic regime where $M_1,\ldots,M_K$ grow proportional to $N$ and define
\begin{align}
...
Then, an i.i.d.\ Gaussian noise with standard deviation $\sigma=3$ was added, inducing an $\SNR$ of approximately $1/20$. A piece of one such micrograph is presented in the right panel of Figure~\ref{fig:micro_example}.
...
In this experiment, we assume the knowledge of the number of Einstein's occurrences and the noise level. 
In this situation, the second-order autocorrelation (or, equivalently, the Fourier magnitudes of the signal) determines   the image  almost surely up to reflection through the origin~\cite{hayes1982reconstruction,bendory2017fourier}. 
Therefore, we estimate the signal's Fourier magnitude from the Fourier magnitude of the micrographs, at the cost of one  FFT per micrograph.
To estimate the signal, we use a phase retrieval algorithm called relaxed-reflect-reflect (RRR) that iterates by~\cite{elser2017matrix}
\begin{equation}
z \rightarrow z + P_2(2P_1(z) - z) - P_1(z).
\end{equation}
The RRR algorithm estimates a $2L\times 2L$ image and the exact solution contains Einstein's image in the upper-left corner and zero elsewhere. The operator $P_2(z)$ combines the Fourier phases of  the current estimation $z$ with the Fourier magnitudes of the signal  (estimated from the data). The operator $P_1(z)$ zeros out all entries of $z$ outside the $L\times L$ upper-left corner. 
In order to compare the performance across multiple cases and different noise levels, the algorithm stopped after a fixed number of 1000 iterations and the iteration with the smallest error compared to the ground truth (also taking into account the reflection ambiguity) was chosen as the solution. While this cannot be done in practice since we do not have access to the ground truth, this procedure enables us to compare a large number of instances in different noisy environments \TODO{TB: Note the last two sentences!}.
...
We do not assume to know the number of occurrences of each signal $\gamma_i$ ahead and we removed the terms depending on $\sigma$ (see~\eqref{eq:data_ac_k1}) so we do not need to know $\sigma$. 
...
To estimate the signal, we employ an optimization algorithm on the following nonlinear least-squares problem that estimates the signals and their number of occurrences simultaneously:
\begin{multline}
...
where $W \geq L$ is the length of the sought signals and $w_1 = \frac{1}{2}, w_2 =\frac{1}{2}(L-1), w_3 = \frac{1}{(L-1)(L-2)}$; see~\cite{boumal2017heterogeneous} see for discussion on how to choose the weights properly.
 ...
 Setting $W = L$  is problematic because the above optimization problems appears to have numerous poor local optimizers.
%Since we can only initialize randomly at first, this approach would often fail in practice. Alternatively,
Thus, we first run the optimization with $W = 2L-1$. This problem appears to have fewer poor local optima, perhaps because the additional degrees of freedom allow for more escape directions. Since we hope the signals estimated this way correspond to the true signals zero-padded to length $W$, we extract from each one a subsignal of length $L$ whose autocorrelation functions are the closest to the measured ones in the sense of~\eqref{eq:optim1D}. 
This estimator is then used as initial iterate for~\eqref{eq:optim1D}, this time with $W = L$. We find that this procedure is reliable for a wide range of experimental parameters. To solve~\eqref{eq:optim1D}, we run the trust-region method implemented in Manopt~\cite{manopt}, which allows enforcement of the positivity constraints \TODO{reference} on the coefficients $\hat \gamma_k$. Notice that the cost function is a polynomial in the variables, so that it is straightforward to evaluate it and its derivatives.
