...
In this section we show numerical results, while leaving the specific details of the experiments for Section~\ref{sec:methods}.
In the first experiment, we estimated Einstein's image from micrographs with $\sigma=3$. An illustrative example for a micrograph appears in the right panel of Figure~\ref{fig:micro_example}.
Multiple occurrences of the image were placed 
in $P$ micrographs, each of of size $4000\times 4000$ pixels, while the images to be separated by the image's length in each direction. From the micrographs, we estimated the first and second order autocorrelation and used a standard phase retrieval algorithm, called relaxed-reflect-reflect, to estimate the image. 
...
To measure error, we use the root mean square error (RMSE) defined as 
\begin{equation}
\text{RMSE}  = \frac{\|x - \hat{x}\|_{\text{F}}}{\|x \|_{\text{F}}},
\end{equation} 
where $\hat{x}$ is the estimated image and $\|\cdot\|_{\text{F}}$ is the Frobenius norm. 
As expected, the RMSE of estimating the power spectrum (an equivalent quantity to the second-order autocorrelation)  decreases linearly with slope $-1/2$ in logarithmic scale. \TODO{What about the error?}
...
Results from a similar experiment for 1-D signals is presented in Figures~\ref{fig:1Dhomosignals} and~\ref{fig:1DhomoRMSE}.
Here, we used nonconvex least-squares (LS) to estimate the signal and the estimated number of its occurrences.
We do not assume the knowledge of the noise level. 
Crucially, starting from the point of $n\approx10^8$, the RMSE of the signal estimation decreases linearly with slope $-1/2$---the expected estimation rate of the autocorrelation functions---implying the stability of the recovery algorithm. \TODO{We may want to replace with an experiment with colored noise}
...
\begin{figure}[t]
	...
	\caption{Red signals are the ground truth (targets) and the blue signals are our estimations. Individual RMSE of the estimates: $0.0239393 / 0.208925 / 0.0335956$. The estimated $\gamma$'s are $0.02574 / 0.01693 / 0.00818$ and the true ones $0.02561 / 0.01707 / 0.00853$. The $\SNR$ is $1/12$.}
