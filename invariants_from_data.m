...
% the signal. For the bispectrum, it is an estimate of the bispectrum of
% the centered signal (mean removed) to avoid bias. If sigma is not
% specified, it will be estimated from the data.
% 
% Note:
% 
% If M is very large, then computing these averages results in
...
% to grow.
    ...
    %% Prepare fft's of centered signals
    Xc_fft = fft(Xc);
    ...
    if nargout >= 3
        ...
        % the parpool has a lot of workers. Turning either of the for loops
        ...
        % sees no other way of doing it with the code as is.)
