...
% contiguous subsignal (with cyclic indexing) of length L_short with best
% loss (the region of interest), and returns it as the column vector
% X_short. Loss is computed from the given function handle lossfun, which
% takes as input a signal of length L_short.

    [L_long, K] = size(X_long);
    assert(K == 1, 'Implemented only for K = 1 for now.');
    assert(L_long >= L_short, 'Can only extract a shorter region.');
    
    % For cyclic indexing, the easiest is to duplicate the signal
    X_long = [X_long ; X_long];
