...
Different variants of the blind deconvolution problem have been subject recently to a through  analysis, focusing on low noise regimes~\cite{ahmed2014blind,li2016identifiability,li2016rapid,lee2017blind,ling2017blind,kuo2019geometry}.
The parameters of the signal $s$ (the locations of its nonzero values) are the \emph{nuisance variables} of the problem. 
The model~\eqref{eq:model} appears in several scientific applications, including spike sorting~\cite{lewicki1998review}, passive radar~\cite{gogineni2017passive} and system identification~\cite{ljung1998system}. 

In the low noise regime, one can estimate $s$ and then estimate $x$ by standard deconvolution techniques. However, in the high noise regime, estimating $s$ is impossible---that is, detection is impossible~\cite{bendory2018toward}. To guarantee recovery in any noise regime, we require in this model a separation between adjacent signal's occurrences, that is,
\begin{equation}
...
\paragraph{The heterogeneous well-separated model.}
The homogeneous well-separated model can be naturally generalized to the problem of estimating a set of $K$ signals $x_1,\ldots,x_K\in\RL$ from their mixed blind deconvolution
\begin{equation} 
y  =  \sum_{i=1}^Kx_i \ast s_i + \varepsilon, \quad  \varepsilon   \sim \mathcal{N}(0,\sigma^2 I_N),
\end{equation}
where again $s_i$ denotes a binary signal indicating the support of the occurrences of $x_i$ in the data. We say that the separation condition is met \TODO{to verify}
\begin{eqnarray}
\text{If } s:=\bigcup\limits_{i=1}^Ks_i \text{ satisfies}~\eqref{eq:spacing} \text{ and for any } i\neq j, \thinspace s_i\cap s_j=\emptyset.
\end{eqnarray}
...
In this model, we let the signal $x\in\RL$ to be a random vector drawn from some fixed distribution. We observe a random vector $Y\in\R^{N-L+1}$. The difference from the previous models is that here in each non-zero element of $s$  a random vector $X$ from the distribution is then placed in the large vector, with element $0$ at location $i$.
This model generalize the homogeneous and heterogeneous models; if the distribution is a Delta function that it coincides with the homogeneous model, and if it is a sum of Diracs then it is the heterogeneous model.
\TODO{We need to write the  separation condition clearly}.

\paragraph{Poisson model with a stochastic signal.}
...
We consider the following observation model. Let $X =\RL$ be a random vector drawn from some fixed distribution.
In this model, points are chosen in $\{1,\dots,N-L+1\}$ according to a Poisson process with parameter $\gamma N$. The parameter $\gamma$ can be thought of as the density of the $X$ in $Y$.
For each point $i$ that is chosen from $1$ to $N-L+1$, a random vector $X$ from the distribution is then placed in the large vector, with element $0$ at location $i$, with overlapping vectors being added together. 

If $M_i$ denotes the number of hits at location $i$, $1 \le i \le N-L+1$, then by definition of the Poisson process $M_i$'s are i.i.d.\ and $M_i \sim \Poisson(\gamma)$. Conditional on the value of $M = (M_1,\dots,M_n)$, if we let $X_1^{i},\dots,X_{M_i}^i$ denote the random vectors with position 0 located at $i$, then $X_{k_1}^{i}$ and $X_{k_2}^{i}$ are independent for $k_1 \ne k_2$.
...
%
\end{align}
\TODO{The well separated model can be also written for a random signal X}

\subsection{Motivation: single-particle reconstruction using cryo-electron microscopy}
...
In contrast X-ray and NMR which exaggerate information from an ensembles of
particles, single particle cryo-EM produces images of individual particles and thus can, in principle, elucidate multiple energy landscapes associated with different structures.
...
In~\cite{henderson1995limitations,glaeser1999electron}), it was established that particle picking is impossible for molecules below a certain weight (below $\sim$50 kDa). 
...
The principle mathematical tool is  autocorrelation analysis, described in detail in Section~\ref{sec:AC_analysis}.
This goal of  the current paper is to provide a theoretical and numerical support  for the method proposed in~\cite{bendory2018toward}.  
In this context, the  models described above serve  as  an abstraction of the cryo-EM problem: the multiple tomographic projections of the 3-D structure (taken from unknown viewing directions) are replaced by one (or multiple) repeated signals. A similar abstraction has been proven itself to be useful in a similar context as described in the next section. 

\subsection{Principle mathematical tool: autocorrelation analysis.}

In order to recover the image, we employ autocorrelation analysis. The underlying principle is to relate the autocorrelations of the micrographs to the autocorrelations of the image, without trying to detect individual signal occurrences (i.e., the signal $s$).
In the models described above, for any noise level, these autocorrelations can be estimated to any desired accuracy. The autocorrelations of the micrographs are straightforward to compute and require only one pass over the data. After estimation of the density of particles in the micrographs, these directly yield estimates for the mixed autocorrelations of the signals. To estimate the signals them from their estimated autocorrelations, we solve a nonlinear inverse problem
...
In order to investigate the computational and statistical properties Kam's method, a series of papers have studied the  multi-reference alignment problem as an abstraction~\cite{bandeira2014multireference,bendory2017bispectrum,bandeira2017optimal,perry2017sample,bandeira2017estimation,abbe2017multireference}.
We follows the same line of research by considering the models of Section~\ref{sec:problem_formulation} as an abstraction to the application of reconstructing 3-D structures directly from the micrograph as proposed in~\cite{bendory2018toward}.
...
\TODO{Could be written for a random signal and then particularized to the homogeneous and heterogeneous cases.}
For a signal $z\in \R^m$, the autocorrelation of order $q = 1, 2, \ldots$ is given for any integer shifts $\ell_1, \ldots, \ell_{q-1}$ by
\begin{equation}
...
\TODO{Start directly with heterogeneous model, but first need to verify that the Poisson equivalence is true holds also for this case.}
...
In this section we omit the affect of the noise as it remains the same for the well-separated model. 

Let us denote by $m_Y^q$ the moments of $Y$:
%
\begin{align}
%
m_Y^1[i] = \E X[i], \quad 0 \le i \le L-1,
%
\end{align}
%
\begin{align}
%
m_Y^2[i,j] = \E X[i] X[j], \quad 0 \le i,j \le L-1,
%
\end{align}
%
and
...
\begin{align}
%
m_Y^3[i,j,k] = \E X[i] X[j] X[k], \quad 0 \le i,j,k \le L-1.
%
\end{align}

%We will also denote by $\L_l$ the autocorrelations of $X$, $l = 1,2,3$:
%%
%\begin{align}
%%
%\L_1 = \sum_{i=0}^{L-1} \M_1[i] \in \R,
%%
%\end{align}
%%
%\begin{align}
%%
%\L_2(\Delta) = \sum_{i=0}^{L-1} \M_2[i,i+\Delta]  \in \R^L,
%%
%\end{align}
%%
%and
...
%\begin{align}
%%
%\L_3(\Delta_1,\Delta_2) = \sum_{i=0}^{L-1} \M_3[i,i+\Delta_1,i+\Delta_2]  \in \R^{L \times L}.
%%
%\end{align}
%
%
...
In the following,  we show that the moments of the Poisson process can be written in terms of the autocorrelations of the well-separated model.

\begin{proposition} \label{prop:poisson}
	The first moment of the Poisson process is equal to:
	%
	\begin{align}
	\gamma \L_1.
	\end{align}
	%
	The second moment vector of the Poisson process has entries
	%
	\begin{align}
	...
	The third moment matrix of the Poisson process has entries
	\begin{align}
	(\gamma \L_1)^3 + \gamma \L_1  \cdot ( \gamma\L_2(\Delta_1) + \gamma\L_2(\Delta_2) + \gamma\L_2(\Delta_2-\Delta_1)) + \gamma \L_3(\Delta_1,\Delta_2) ,
	\end{align}
	for $0 \le \Delta_1,\Delta_2 < L$.
\end{proposition}


\begin{corollary}
	From the first three moments of the Poisson process model, once can recover the first three moments from the strongly-separated model.
\end{corollary}

%%%, with the Poisson rate $\gamma$ playing the role of the ``occupancy factor''. So if recovery is possible for the strongly-separated model, it is also possible for the Poisson process model.
...
\begin{equation}
a_z^q[\ell_1,\ldots,\ell_{q-1}]   = \frac{1}{m^d} \sum_{i\in\mathbb{Z}^d} z[i]z[i+\ell_1]\cdots z[i+\ell_{q-1}].
\label{eq:ac_d_dimension}
\end{equation}
Interestingly, for dimensions greater than one almost all signals are determined uniquely from their second-order autocorrelation, up to two symmetries: sign (or phase for complex signals) and reflection through the origin (with conjugation in the complex case)~\cite{hayes1982reconstruction}. 
This is not true for 1-D signals: generally there are $2^{L-2}$ signals with the same second-order autocorrelation (after eliminating the symmetries)~\cite{beinert2015ambiguities,bendory2017fourier}. 

In the literature, the phase and reflection symmetries are usually called trivial ambiguities. If the mean of signal is available and non-zero, the sign symmetry can be resolved. However, determining the reflection symmetry still requires additional information, beyond the second-order autocorrelation.
...
Hence, if $\sigma$ is know we have 
\begin{align*}
...
\subsection{The first moment of $Y$}
...
\subsection{The second moment of $Y$}
...
\subsection{The third moment of $Y$}
