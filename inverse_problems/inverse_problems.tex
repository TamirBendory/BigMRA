...
We consider the problem of recovering a set of $K$ signals that appear 
...
Let  $s \in \mathbb{N}^{N-L+1}$. In the $i$th entry of $s$, $s[i]$ signals are positioned in $y$ with element $0$ at location $i$, each of those  is chosen according to some (possibly unknown) distribution over $\{1,\ldots,K\}$. 
If signal occurrences overlap, they interfere additively. %We think of those events as being rare. 
With additive white Gaussian noise, the measurement model can be written as a mix of blind deconvolution problems 
\begin{equation} 
