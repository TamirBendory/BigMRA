...
In the high noise regime neither detection nor  clustering can be performed reliably and thus such a strategy is destined to fail.  
By employing autocorrelation analysis, we show that in some cases the signals themselves can be estimated  without detecting their occurrences. 
To estimate the signals, we derive simple relations between the autocorrelations of the observations and those of the signals. These autocorrelations can be estimate accurately in any noise level given sufficiently long measurement. 
...
The main goal of this work is to supply theoretical and numerical support for a recent suggestion to  estimate the 3-D structure of single particles using cryo-electron microscopy in extreme noise levels~\cite{bendory2018toward}.
