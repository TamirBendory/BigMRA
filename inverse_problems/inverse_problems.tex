...
The recovery entails solving a system of low-order polynomial equations; the properties of the specific systems  (e.g., its condition number) affects the feasibility of the estimation procedure.
...
In order to gain better understanding of the method, this paper focused on an abstraction of cryo-EM---the multi-target detection model. 
Our next goal is to reconsider the full cryo-EM model, both from theoretical and algorithmic perspectives. 
In particular, the numerical results in~\cite{bendory2018toward} suggest that the achieved resolution using the third-order autocorrelation is limited;  higher resolution may require computing high-order autocorrelations, which will increase the sample complexity of the problem and the algorithm's computational complexity. 
