...
Specifically, 
 letting $s[i]$ be the number of signals starting at location $i$, ${s[i] \overset{i.i.d.}{\sim} \Poisson(\gamma / L)}$  for some parameter $\gamma > 0$. 
...
\begin{align} \label{eq:ac_well_separated}
	a_y^1 & = \gamma a_{x}^1, \nonumber \\
	a_y^2[\ell] & = \gamma a_{x}^2[\ell] + \sigma^2\delta[\ell], \\
...
that $x,\gamma$ and $\sigma$ can be identified uniquely from the autocorrelations for the homogeneous case where $K = 1$ (all signal occurrences are the same).
...
In Section~\ref{sec:proof_prop_poisson}, we prove the following expressions for the autocorrelations of $y$ under the Poisson model, expressed in terms of the autocorrelations of $x$ and the parameters $\gamma$ and $\sigma$:
%
\begin{align}
a_y^1  &=  \gamma a_{x}^1, 
...
In the homogeneous case ($K=1$), we show in Section~\ref{sec:theory_homogeneous_poisson} that $a_y^1,a_y^2$ and $a_y^3$ identify the signal $x$, the Poisson parameter $\gamma$ and the noise level $\sigma$ for generic $x$. 
%
...
A signal $z\in\RL$ is determined uniquely by its second- and third-order autocorrelations. Indeed, assuming $z[0]$ and $z[L-1]$ are nonzero (otherwise, redefine the length of the signal), we can recover $z$ explicitly using this identity for $k = 0, \ldots, L-1$:
%
\begin{equation}
%
...
The observed moments $a_y^1,a_y^2$ and $a_y^3$ under the well-separated model~\eqref{eq:ac_well_separated} do not immediately yield the autocorrelations  of the signal $x$; rather, the two are related by the noise level $\sigma$ and the ratio $\gamma$. We will show, however, that $x$ is still determined by the observed moments of $y$.
...
\begin{proposition} \label{prop:gamma}
	Let $\sigma > 0$ be fixed and assume that the separation condition~\eqref{eq:spacing} holds. If the mean of $x$ is nonzero, then  \TODO{Should remove the limit}
	\begin{equation}
...
The proof is provided in Section~\ref{sec:proof_uniqueness_poisson}.
...
\TODO{We write the section for the Poisson model, but it would be the same for the Poisson model}
...
\begin{align}
%
a_x^2[\Delta] = \frac{1}{L} \sum_{i=0}^{L-1} \M_2[i,i+\Delta],
%
\end{align}
%
and
...
\begin{align}
%
a_x^3[\Delta_1,\Delta_2] = \frac{1}{L}\sum_{i=0}^{L-1} \M_3[i,i+\Delta_1,i+\Delta_2].
%
\end{align}
...
\begin{lem} \label{lem-choose}
	%
	If $M \sim \Poisson(\lambda)$, then 
	\begin{align}
	%
	\E {M\choose k} = \frac{\lambda^k}{k!}.
	%
	\end{align}
\end{lem}
...
We will first condition on the vector $s$ of locations of the subsignals in $y$, and then average over $s$. We will denote by $X_1^{i},\dots,X_{S[i]}^i$ the random vectors with position 0 located at $i$. We have:
%
\begin{align}
%
\E[ Y[i] | S ] = \sum_{j=0}^{L-1} \sum_{k=1}^{S[i-j]} \E X_k^{i-j}[j]
...
\nonumber \\
=& \sum_{j_1 - j_2 \ne \Delta} S[i_1-j_1] S[i_2 - j_2] \M_1[j_1] \M_1[j_2]
\nonumber \\
& + \sum_{j_1 = 0}^{L-1} S[i_1-j_1](S[i_1-j_1] - 1) \M_1[j_1] \M_1[j_1 + \Delta]
\nonumber \\
& + \sum_{j_1 = 0}^{L-1} S[i_1-j_1] \M_2[j_1,j_1 + \Delta] .
%
\end{align}
%
...
%
\E Y[i_1] Y[i_2]
=& \sum_{j_1 - j_2 \ne \Delta} \E S[i_1-j_1] S[i_2 - j_2] \M_1[j_1] \M_1[j_2]
\nonumber \\
&       + \sum_{j_1 = 0}^{L-1} \E S[i_1-j_1](S[i_1-j_1] - 1) \M_1[j_1] \M_1[j_1 + \Delta]
\nonumber \\
&       + \sum_{j_1 = 0}^{L-1} \E S[i_1-j_1] \M_2[j_1,j_1 + \Delta]
\nonumber \\
=& \frac{1}{L^2}\sum_{j_1 - j_2 \ne \Delta} \gamma^2 \M_1[j_1] \M_1[j_2]
+ \frac{1}{L^2} \sum_{j_1 = 0}^{L-1} \gamma^2 \M_1[j_1] \M_1[j_1 + \Delta]
\nonumber \\
&       + \frac{1}{L}\sum_{j_1 = 0}^{L-1} \gamma \M_2[j_1,j_1 + \Delta]
\nonumber \\
=&  \bigg(\frac{\gamma}{L}  \sum_{j = 0}^{L-1} \M_1[j] \bigg)^2
+ \frac{\gamma}{L} \sum_{j = 0}^{L-1} \M_2[j,j + \Delta]
\nonumber \\
=&  (\gamma a_x^1)^2 + \gamma a_x^2[\Delta].
%
\end{align}
%
...
= (\gamma a_x^1)^2 + \gamma a_x^2[\Delta] + \sigma^2 \delta[i_1 - i_2].
\end{align}
%
We conclude by averaging over $i_1$ and $i_2$, leaving the gap $\Delta = i_2 - i_1$ fixed.
...
We will first assume $\sigma = 0$. For three distinct $i_1$, $i_2$ and $i_3$, we let $\Delta_1 = i_2 - i_1$ and $\Delta_2 = i_3 - i_1$. We have:
%
\begin{align}
%
& Y[i_1] Y[i_2] Y[i_3]
...
\begin{align}
%
\sum_{j=0}^{L-1} S[\ell](S[\ell]-1)(S[\ell]-2)\M[j] \M[j+\Delta_1] \M[j+\Delta_2].
%
\end{align}
%
...
%
\frac{\gamma^3}{L^3} \sum_{j=0}^{L-1} \M_1[j] \M_1[j+\Delta_1] \M_1[j+\Delta_2].
%
\end{align}
...
\begin{align}
%
\sum_{j=0}^{L-1} S[\ell] (S[\ell] - 1) \M_2[j,j+\Delta_1] \M_1[j + \Delta_2]
%
\end{align}
%
...
%
\frac{\gamma^2}{L^2} \sum_{j=0}^{L-1} \M_2[j,j+\Delta_1] \M_1[j+\Delta_2].
%
\end{align}
...
%
\frac{\gamma^2}{L^2} \sum_{j=0}^{L-1} \M_2[j,j+\Delta_2] \M_1[j+\Delta_1].
%
\end{align}
%
and
...
%
\frac{\gamma^2}{L^2} \sum_{j=0}^{L-1} \M_2[j+\Delta_1,j+\Delta_2] \M_1[j].
%
\end{align}
...
%
\frac{\gamma}{L} \sum_{j=0}^{L-1} \M_3[j,j+\Delta_1,j+\Delta_2].
%
\end{align}
%
...
\sum_{1 \le k_1,k_2 \le S[\ell_1]} \sum_{k_3=1}^{S[\ell_2]}
X_{k_1}^{\ell_1}[j_1] X_{k_2}^{\ell_1}[j_1 + \Delta_1] X_{k_3}^{\ell_2}[j_3]   
\nonumber \\
=& \sum_{j_1=0}^{L-1} \sum_{j_3 \ne j_1 + \Delta_2} \sum_{k_3=1}^{S[\ell_2]}
\Bigg\{  \sum_{1 \le k_1 \ne k_2 \le S[\ell_1]} 
X_{k_1}^{\ell_1}[j_1] X_{k_2}^{\ell_1}[j_1 + \Delta_1] X_{k_3}^{\ell_2}[j_3]
\nonumber \\
& + \sum_{k_1=1}^{S[\ell_1]} X_{k_1}^{\ell_1}[j_1] X_{k_1}^{\ell_1}[j_1 + \Delta_1] 
...
%
\sum_{j_1=0}^{L-1} \sum_{j_3 \ne j_1 + \Delta_2} 
\Bigg( & S[\ell_1] (S[\ell_1]-1) S[\ell_2] \M_1[j_1] \M_1[j_1 + \Delta_1] \M_1[j_3]
\nonumber \\
&  + S[\ell_1] S[\ell_2] \M_2[j_1,j_1+\Delta_1] \M_1[j_3] \Bigg).
%
\end{align}
...
Finally, in Case \ref{case5} we have $i_1 - j_1$, $i_2 - j_2$, and $i_3 - j_3$ are all pairwise distinct. Consequently, the $X$ variables are always independent, and the expectation conditional on $S$ (letting $\ell_q = i_q - j_q$, $q=1,2,3$),
%
\begin{align}
%
\sum_{j_1,j_2,j_3} S[\ell_1] S[\ell_2] S[\ell_3] \M_1[j_1] \M_1[j_2] \M_1[j_3];
%
\end{align}
%
since the $S[\ell_q]$'s are pairwise independent, $q=1,2,3$, the expectation over $S$ then yields:
%
...
%
\gamma a_x^1  \cdot 
( \gamma a_x^2[\Delta_1] + \gamma a_x^2[\Delta_2] + \gamma a_x^2[\Delta_2-\Delta_1]).
%
\end{align}
%
...
We conclude by averaging over $i_1$, $i_2$, and $i_3$ leaving the gaps $\Delta_1 = i_2 - i_1$ and $\Delta_2 = i_3 - i_1$ fixed.
%
...
\subsection{Proof of Proposition~\ref{prop:uniqueness_poisson}} \label{sec:proof_uniqueness_poisson}
	...
	It is now easy to show how to solve for both $\gamma$ and $\sigma$ from these expressions, assuming $x$ is generic. Indeed, we observe the product (where $x[j]=0$ whenever $j < 0$ or $j > L-1$):
	%
	\begin{align}
...
The authors  thank Ayelet Heimowitz, Joe Kileel, Ti-Yen Kan and Amit Moscovich  for helpful discussions..
