...
The homogeneous well-separated model can be naturally generalized to the problem of estimating a set of $K$ signals $x_1,\ldots,x_K\in\RL$. Let $s$ be a binary signal satisfying the separation condition~\eqref{eq:spacing}. Let $s[i]$ be one of the non-zero values of $s$. A random variable $v_i$ is drawn from some (possibly unknown) distribution over $\{1,\ldots,K\}$. 
Then $x_i$ is placed in $y$
...
\end{equation}
where again $s_i$ is a binary signal, which is non-zero values 
...
This model generalize the homogeneous and heterogeneous models. 
We let the signal $x\in\RL$ to be a random vector drawn from some fixed distribution. We observe a random vector $Y\in\R^{N-L+1}$. For each non-zero element of $s$,  a random vector $X$ from the distribution is then placed in the large vector, with element $0$ at location $i$.
If the distribution is a Delta function that it coincides with the homogeneous model; if it is a sum of Diracs then it is the heterogeneous model.
...
In the models described above, for any noise level, these autocorrelations can be estimated to any desired accuracy. The autocorrelations of the observation are straightforward to compute and require only one pass over the data. After estimation of the density of particles in the micrographs, these directly yield estimates for the mixed autocorrelations of the signals. To estimate the signals them from their estimated autocorrelations, we solve a nonlinear inverse problem as explained in Section~\ref{sec:numerics}. \TODO{to re-write}
...
\label{eq:ac_general}
\end{equation}
where the expectation is taken with respect to the distribution of $Z$ and
the indexing of $Z$ out of the range $0, \ldots, m-1$ is zero-padded.
...
\label{eq:gamma}
\end{equation}
denote the density of $X$ in $y$ (that is, the fraction of entries of $y$ occupied by occurrences of $X$.) The separation condition imposes $\gamma\leq\frac{L}{2L-1}\approx 1/2$.
...
\lim_{N\to\infty} a_y^1  \aseq \gamma \E\left\{a_{X}^1\right\},
\end{equation}
...
Thus, given enough data, if $\gamma$ is known, we can estimate $\E\left\{a_{X}^1\right\}$ from $y$. (We show later how to estimate $\gamma$ as well.) 
...
\end{equation}
for $\ell = 0, \ldots, L-1$, where $\delta[\ell]$ equals one for $\ell=0$ and zero otherwise. The last part captures the autocorrelation of the noise. Notice that, even if $\sigma$ is unknown, entries $\ell = 1, \ldots, L-1$ still provide useful information about $\E\left\{a_{X}^2[\ell]\right\}$.

Along the same lines, one can establish a relation for third-order autocorrelations:
\begin{equation}
 \label{eq:ac3_micrograph}
\lim_{N\to\infty} a_y^3[\ell_1,\ell_2] \aseq \gamma \E\left\{a_{x}^3[\ell_1,\ell_2]\right\}  + \sigma^2\gamma \E\left\{a_{x}^1\right\}  \big(\delta[\ell_1,0]+\delta[0,\ell_2]
