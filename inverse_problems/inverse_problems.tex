...
Until now we have presented models assuming that the signals' occurrences are well separated. This condition can be alleviated by assuming a Poisson distribution on $s$. 

Let $X\in\RL$ be a random vector drawn from some fixed distribution. 
We observe a random vector $Y\in\R^N$ generated as follows. Points are chosen in $\{0,\dots,N-L\}$ according to a Poisson process with parameter $\gamma N$. 
The parameter $\gamma$ plays the role of the density of the signal in the measurement. 
For each point $i$ that is chosen from $1$ to $N-L$, a random vector $X$ from the distribution is then placed in the large vector, with element $0$ at location $i$, with overlapping vectors being added together. 
If the distribution of $X$ is a single Dirac function, then we observe multiple repetition of the same signals, like in the homogeneous case. If the distribution consists of $K$ Diracs, then it is a heterogeneous model. 

If $M_i$ denotes the number of hits at location $i$, $1 \le i \le N$, then by definition of the Poisson process $M_i$'s are i.i.d.\ and $M_i \sim \Poisson(\gamma)$. Conditional on the value of $M = (M_1,\dots,M_N)$, if we let $X_1^{i},\dots,X_{M_i}^i$ denote the random vectors with position 0 located at $i$, then $X_{k_1}^{i}$ and $X_{k_2}^{i}$ are independent for $k_1 \ne k_2$.
...
\TODO{Maybe we should re-write this section for a random signal and then particularized to the homogeneous and heterogeneous cases}
...
In this section we omit the effect of the noise as it remains identical to the well-separated model. 

The moments of $Y$ are defined by
%
\begin{align}
%
m^1_Y[i] = \E Y[i], \quad 0 \le i \le L-1,
%
\end{align}
%
\begin{align}
%
m^2_Y[i,j] = \E Y[i] Y[j], \quad 0 \le i,j \le L-1,
%
\end{align}
%
and
...
\begin{align}
%
m^3_Y[i,j,k] = \E Y[i] Y[j] Y[k], \quad 0 \le i,j,k \le L-1.
%
\end{align}
...
%\end{align}
%
%Note that in the strongly-separated model, the first three observed moments are, respectively, $\L_1$, $\L_2(\Delta)$, and $\L_3(\Delta_1,\Delta_2)$. \TODO{I think we can remove it}
%%
%
We now prove that the first three autocorrelations of the well-separated model can be recovered from the moments of the Poisson process. 

\begin{proposition} \label{prop:poisson}
Let $a_X^1, a_X^2$ and $a_X^3$ be the first three autocorrelation function of the well-separated model \TODO{This is a bit unclear now, but if I get it right it holds true for any distribution over $X$}. Then, for any   $0\leq i\leq L-1$ we have 
\begin{align}
m_Y^1[i] &= \gamma a_X^1,  \nonumber \\
m_Y^2[i,i+\ell] &= (\gamma a_X^1)^2 + \gamma a_X^2[\ell], \quad \ell\in[0,L-1], \\
m_Y^3[i,i+\ell_1,i+\ell_2] &= 
(\gamma a_X^1)^3 + \gamma a_X^1  \cdot ( \gamma a^2_X[\ell_1] + \gamma a_X^2[\ell_2] + \gamma a_X^2[\ell_2-\ell_1]) + \gamma a_X^3[\ell_1,\ell_2], \quad \quad \ell_1,\ell_2\in[0,L-1]. \nonumber
\end{align}
\end{proposition}
