...
This paper suggests a computational  framework for estimation in extreme noise levels. 
The crux of the method lies in the distinction between parameters that we aim to estimate (the signals) and nuisance variables (parameters associated with signals' individual occurrences). In particular, we show that estimating the signals might be possible, although we cannot detect signal occurrences in the data. The method consists of two steps. First, we estimate the autocorrelations of the observation; 
these autocorrelations can be estimated to any desired accuracy 
 given sufficiently long observation. 
Second, the signal is recovered from the autocorrelations. 
The recovery entails solving a system of low-order polynomial equations; the properties of the specific systems  (e.g., condition number) affects the feasibility of the estimation procedure.

The prime motivation of this paper emanates from the problem of reconstructing small particles using cryo-EM.
Small particles induce low signal-to-noise ratio on the micrograph, which  in turn implies that particle picking---the first step in any current cryo-EM reconstruction algorithm---is impossible. 
The main message of our recent manuscript~\cite{bendory2018toward} is that  particle picking is merely an intermediate step (although frequently very useful) and the location of individual projections are nuisance variables. The ultimate goal is to estimate the 3-D structure. 
To this end,  we employed autocorrelation analysis to estimate the structure directly from the micrograph, without particle picking. 
The numerical results in~\cite{bendory2018toward} suggest that the achieved resolution using the third-order autocorrelation is limited;  higher resolution may require computing high-order autocorrelations, which will increase the sample complexity of the problem and computational complexity of the method. 


Autocorrelation analysis enables a flexible framework to incorporate additional parameters into the model.
This is of particular importance for cryo-EM.
The cryo-EM model includes, in addition to the target volume, the 3-D orientation distribution of molecules in the ice, the distribution of projections across the  micrograph, the noise power spectrum (which is usually colored) and  the microscope contrast-transfer function.
These parameters will be encoded within the estimated autocorrelations.
If those parameters are unknown, they should  be  estimated, simultaneously with the 3-D structure, by solving the polynomial system of equations. 
