...
\significancestatement{It is commonly believed that cryo-EM cannot be used to image the 3-D structure of small molecules (below ~50kDa). The classical argument is that, due to the high noise levels in cryo-EM, small particles cannot be detected reliably in micrographs. Challenging that belief, we demonstrate on a mathematical toy model that detection is not a prerequisite for structure reconstruction. Our reconstruction method is based on autocorrelation analysis. It requires only one pass over the data, so that it should, in principle, be appropriate for high-throughput experiments. This autocorrelation approach could also apply to other applications where the goal is estimation rather than detection.
}
...
\correspondingauthor{\textsuperscript{2}To whom correspondence should be addressed. E-mail: tamir.bendory@princeton.edu}
...
\begin{figure*}[h]
	...
	\begin{subfigure}[h]{0.24\textwidth}
		...
		\caption{$500$ micrographs \TODO{Tamir: I am redo the exp}}
	\end{subfigure} 
	\begin{subfigure}[h]{0.24\textwidth}
		\centering
...
\revised{The code to generate all figures is publicly available in TKTK.}
...
First, we assume an additive white noise model, while in practice the noise may be colored.
In that case, the general strategy would remain the same: establish a forward model expressing the expected autocorrelations of the micrographs in terms of the target volume, its density in the micrograph and the noise statistics (which would now involve more than the single parameter $\sigma^2$).
...
\TODO{We need to add running times}
