...
\TODO{Nico: At this point in the paper, the reader will want to here about recovering $x$ from its moments. That's not what happens: the next section is called theory (a bit too vague); it discusses two things: one is the impossibility of getting $s$, another is whether we can get $x$, $\gamma$, $\sigma$. I propose to split that section in two, and to lead with the recovery part (which is interesting to more people, I expect.)}
...
\begin{proposition} \label{prop:two_gauss}
	...
	\end{align*}
	that is: the probability of success is no better than random chance in the limit of infinitely poor SNR.
\end{proposition}
...
Proposition~\ref{prop:two_gauss} implies that in order to estimate the signal in low SNR  we must consider methods that aim to estimate the signal directly, without estimating the nuisance variables as an intermediate step. 
 %we need to consider a different approach. In this paper, we consider autocorrelation analysis.
  In what follows, we present several results on properties of autocorrelations. \TODO{For simplicity, we consider one-dimensional signals, but the results carry through for higher dimensions.---need to keep this sentence in red until the very end to make sure it's correct.}
