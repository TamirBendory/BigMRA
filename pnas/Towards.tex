...
%% Nicolas: this is too important: it must stand out
% If $s[i] = 1$ and $s[j] = 1$  for  $i \neq j$, then $\vert i - j\vert \geq 2L-1$.
\begin{align} 
...
%Indeed, for a general support signal $s$, on each iteration of EM, a probability must be assigned to any feasible combination of positions for the current signal estimate in $M$ locations on the grid $\{1,\ldots,N\}$---$O(N^M)$ such combinations in total. Hence, the problem becomes computationally intractable when $M$ grows with $N$ and $N$ is large. However, exploiting the separation condition on the support~\eqref{eq:spacing}, we devise in Section TKTK an approximate EM procedure for  our model. 
%
...
To support this claim, we consider a strictly simpler problem: suppose an oracle identifies for us intervals of length $L$ in the micrograph that either contain a full signal occurrence (plus noise), or contain just noise. Our task is to determine which intervals contain signal, that is, to determine whether the corresponding entries of $s$ are 0 or 1.
The oracle further provides the signal $x$, the probability $q$ that one of the intervals contains signal, and the noise variance $\sigma^2$. \TODO{Nico: I don't see how the next sentence is relevant for the task of estimating $s$---Since we have the full generative model, we can always generate more data as needed; therefore it is enough to consider the hardness of deciding whether a single interval contains signal or not.---I would just write this: Since the intervals are independent of each other, it is sufficient to consider a single interval.}

The decision problem for a single interval can be abstracted as follows: We have two known vectors $\theta_0 = x$ and $\theta_1 = 0$ in $\mathbb{R}^L$. There is a random variable $\eta$ taking values 0 or 1 with probabilities $q$ and $1-q$, respectively. We observe a random vector $X \in \mathbb{R}^L$ (an extract of the micrograph) with the following distribution:
% $X | (\eta  = i) \sim N(\theta_i,\sigma^2)$; in other words,
if $\eta = 0$, then $X \sim \mathcal N(\theta_0,\sigma^2)$ (that is, $X$ follows a normal distribution around $\theta_0$ with variance $\sigma^2$), and if $\eta = 1$, then $X \sim \mathcal N(\theta_1,\sigma^2)$. \TODO{Tamir: to remove parenthesis}
%If $\theta_1=0$, then the latter corresponds to pure noise interval.

We observe $X$, and our task is to estimate $\eta$. How reliably can this be done? If $q \geq 1/2$, the constant estimator $\hat{\eta} = 0$ is correct with probability $q$; likewise for $\hat{\eta} = 1$ if $q \leq 1/2$, correct with probability $1-q$. The question is, can we do better than this? \TODO{to rephrase} We prove that, as $\sigma \to \infty$, the answer is no:
...
%we need to consider a different approach.
In the next sections, % Nicolas: sections is plural on purpose
we consider autocorrelation analysis for that purpose.
%In what follows, we present several results on properties of autocorrelations. 
...
Under the separation condition on $s$,  for shifts in $0, \ldots, L-1$, the autocorrelation functions of $y$ depend only on the corresponding autocorrelations of $x$, the density $\gamma$ and the noise level $\sigma$. In particular, for the first-order autocorrelation (that is, the mean), since the noise is independent of the signal, the mathematical expectation of $a_y^1$ is easily seen to be:
\begin{align*}
\E\{ a_y^1 \} & = \gamma a_x^1.
\end{align*}
...
\begin{multline}
\lim_{N\to\infty} a_y^3[\ell_1,\ell_2] = \gamma a_{x}^3[\ell_1,\ell_2] \label{eq:ac3_micrograph} \\ + \sigma^2\gamma a_{x}^1 \cdot \big(\delta[\ell_1,0]+\delta[0,\ell_2]+\delta[\ell_1,\ell_2]\big),
\end{multline}
...
In particular, if the separation condition holds \TODO{for some spacing $W\geq L$---Nico: what is this?}, then $a_z^2[i] = 0$ for all $i > L-1$.
...
However, for dimensions greater than one, almost all signals are determined uniquely up to sign (or phase for complex signals) and reflection through the origin (with conjugation in the complex case). %~\cite{hayes1982reconstruction,hayes1982reducible}. 
...
\begin{proposition} \label{prop:gamma}
	Let $\sigma > 0$ be fixed. If the mean of $x$ is nonzero, then  \TODO{Where does the `almost surely' come from?}
	%
	\begin{align*}
	%
	\gamma & = \lim_{N \to \infty}\frac{(a^1_y)^2}{\sum_{\ell = 0}^{L-1}a_y^2[\ell]-\sigma^2}, \quad \text{almost surely.}
	%
	\end{align*}
	\TODO{Nico: I do not get the same formula when I plug things in.}
	%
\end{proposition}
\begin{proof}
...
lies in a set \TODO{with Lebesgue measure zero.---Nico: I don't think we should use such words in a PNAS paper.}
...
\begin{figure*}[h!]
	...
	\caption{\label{fig:1D_example} Recovery of a 1-D signal of length $L=21$ at noise level $\sigma = 3$ from $M$ repetitions. The length of the micrograph was set to be $N = 410*M$. The first three panels (left to right) show reconstruction with different $M$ values compared to the ground truth signal (in blue).
		The true value of $\gamma$ is $0.0512$.
		   The relative errors of $\gamma$ for the three panels are: $27.1\%,4\%,1.2\%$. The right panel shows the relative error as a function of $M$.  \TODO{Nico: relative error should be logscale? Also, let's pick other colors; if we're running this again anyway, let's also aim for M being powers of 10 in all displayed cases?}
	}
\end{figure*}
...
\begin{figure*}[h]
	...
	\caption{\label{fig:cryo_detection} All micrographs are of size $7420^2$ pixels and the projections are taken from a volume of size $31^3$. The added noise was drawn from i.i.d.\ Gaussian distribution with zero mean and standard deviation $25$, corresponding to an SNR below $1/1024$.  
		The noise realization is identical in both micrographs. \TODO{The excerpts should be squares: use `axis equal;' -- also, what's going on with the diagonal line on the right panel?}}
\end{figure*}

\begin{figure*}
    \centering
    \includegraphics[angle=270, width=0.3\textwidth]{TRPV1_rec.eps}
    \caption{Reconstruction of the TRPV1 molecule from its clean first 3 autocorrelations. The reconstruction is shown in gray while the ground truth is shown in yellow.}
    \label{fig:TRPV1_recon}
\end{figure*}
...
\TODO{Fill in refs to eqns.} For the experiment in Figure~TKTK, we computed the analytical first three autocorrelations of the volume according to~TKTK. To estimate the coefficients of the volume itself, we solve the LS problem
...
The LS problem was solved using Matlab's \texttt{lsqnonlin} solver for nonlinear least-squares problems. The expected $\hat \gamma$ in this experiment is one.

The true volume used in both experiments was the Bovine Pancreatic Trypsin Inhibitor (BPTI) mutant with altered binding loop sequence, whos atomic model is available in the Protein Data Bank (PDB)~\cite{pdb} as 1QLQ. We generated an EM map from this atomic model in UCSF Chimera~\cite{pettersen2004ucsf} at a resolution of 5 \AA, and cropped it to remove zeros at the boundary to obtain a volume of size $31^3$. We also present a reconstruction of TRPV1 in complex with DkTx and RTX, whose EM map is available in the  Electron Microscopy Data Bank (EMDB)~\cite{emdb} as EMD-8117. The original map has size $192^3$, and we downsample it to size $20^3$. To generate the ground truth for our reconstructions, we expanded both volumes as in \eqref{eq:volume_expansion_app} with cutoff $L = 5$. For the TRPV1 reconstruction, the optimizer converged to a point giving relative $\ell_2$ error of $10^{-6}$ in the first three autocorrelations and an error of $10^{-1}$ in the expansion coefficients of the volume. Similarly, \TODO{Fill details for BPTI}. This illustrates the ill-conditioning of the map between the volume and its first three autocorrelations that prevents us from obtaining results from noisy data.

The micrographs for the experiment presented in Figure~\ref{fig:cryo_detection} were generated as follows. We sample rotation matrices from SO(3) uniformly at random using the QR-based algorithm described in QR, and generate the projection of the volume corresponding to that rotation matrix using \verb|cryo_project| in ASPIRE \cite{aspire}. We keep track of the indices at which the upper left corner of a projection can be placed without violating the separation condition, so all projections are separated by at least 30 pixels in each dimension. We continue adding projections to the micrograph until no more projections can be added without violating the separation condition. Then, the LS problem~\eqref{eq:LS_cryo} was solved assuming the spherical harmonic cutoff for the volume is $L=0$, which is sufficient to recover a significant $\widehat\gamma$ in the presence of projections in the micrograph.
...
We will let the number of instances of the signal $M$ grow with $N$, and write $M=M_N$ to emphasize this. We assume $M_N$ grows proportionally with $N$, and define: \TODO{---Nico: the paper now no longer references $M_N$ and defines $\gamma$ 'as is' for given $M$ and $N$; may want to adapt things here as a result. It wouldn't make sense to make $M_N$ appear in the paper I think, as it just makes things more complicated. In a sense, much of this beginning of appendix is just repeating things we already said cleanly in the paper: let's just follow up from that.}
%
\begin{equation}
...
The proof is based on  a variant of the Neyman--Pearson Lemma to derive the best (deterministic) estimator $\hat{\eta}$. We take any estimator $\hat{\eta}$, and we let $S$ be the set of $X$'s where $\hat{\eta} = 1$. We write $\P_i$ to mean the probability conditional on the event $\eta = i$; that is, $\P_i[A] = \P[A | \eta = i]$. Then, the probability that $\hat{\eta}$ fails is:
%
\begin{align}
...
The best estimator of $\eta$ based on $X$ minimizes the failure probability; hence, it minimizes the integral in~\eqref{eq:neyman-pearson} through an appropriate choice of the set $S$. This is achieved as follows:
\begin{align}
%
%S & = \left\{x : \frac{f_0(x)}{f_1(x)} \leq \frac{1-q}{q} \right\}.
S & = \left\{x : qf_0(x) \leq (1-q)f_1(x) \right\}.
%
\end{align}
%
%In other words, if $\Lambda(x) = f_0(x) / f_1(x)$ and $b = (1-q) / q$, then the best estimator of $\eta$ based on $X$ is:
%
...
\end{align*}
%
Now let us compute the probability of failure conditional on the event $\eta = 0$. In this case, failure occurs when $X \in S$. Since $X|(\eta=0) \sim N(\theta_0,\sigma^2)$, we can write $X | (\eta=0) = \sigma Z + \theta_0$, where $Z \sim N(0,1)$. On that condition,
%
\begin{align*}
%
\langle X , \theta_1 - \theta_0 \rangle 
...
%where $k=||\kk||_2$, 
where $c$ is the bandlimit, $\{S_l\}$ are determined using the Nyquist criterion as descirbed in \cite{bhamre2017anisotropic}, $j_{\ell,s}$ is the normalized spherical Bessel functions given by
...
Because $\phi$ is real-valued, so $\widehat \phi$ is conjugate-symmetric, the expansion coefficients satisfy $\tamir_{\ell, -m, s} = (-1)^{\ell+m}\overline{\tamir_{\ell,m,s}}$, and hence we only need to recover coefficients $\tamir_{\ell, m, s}$ with $m\geq 0$. Let $I_{\omega}$ denote the tomographic projection obtained from viewing direction $\omega\in SO(3)$. By the Fourier projection-slice theorem, its 2-D Fourier transform  is given by~\cite{natterer1986mathematics}:
...
&= \sum_{\ell,m,m',s}\tamir_{\ell,m,s}\left[\frac{2\sqrt{2\pi}}{\alpha_{N,n}}Y_{\ell}^{m'}(\pi/2,0)\right]D_{m',m}^{\ell}(\omega)\\&\times \left(\int_0^1j_{\ell,s}(k)R_{N,n}(k)k\, dk\right)\left(\frac{1}{2\pi}\int_0^{2\pi}e^{\iota(m'-N)\theta}\, d\theta\right),\\
&= \sum_{\vert N\vert\leq \ell}\sum_{m,s}\tamir_{\ell,m,s}D_{N,m}^{\ell}(\omega)\beta_{\ell,s;N,n},\end{aligned}\]
where the coefficients
\begin{equation} \label{eq:beta}
\beta_{\ell,s;N,n} := \left\{\begin{array}{ll} \frac{2\sqrt{2\pi}}{\alpha_{N,n}}Y_{\ell}^{N}(\pi/2,0)\int_0^1j_{\ell,s}(k)R_{N,n}(k)k\, dk, & \ell\geq |N|\\ 0, & \ell<|N|\end{array}\right.,
\end{equation}
can be precomputed.
