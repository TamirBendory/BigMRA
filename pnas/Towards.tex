...
\begin{figure}[h]
	...
	\caption{\label{fig:cryo_recon} Reconstructions from the first, second and third order autocorrelations.
		...
		\revised{The original molecules are shown in purple
				and the (smoothed) ground truths in blue to illustrate the smoothing effect of our downsampling and truncation of the spherical harmonics expansion.
			The reconstructions  from clean autocorrelations are shown in yellow, and 	recovery from autocorrelations estimated from noisy data in gray. For the noisy experiments, we used 300 micrographs with SNRs of 1/16 for TRPV1 and 1/64 for BPTI. We present excerpts from noisy micrographs side-by-side with the corresponding clean ones in Figure~\ref{fig:cryo_noisy_micros_for_rec}}.}
		 \vspace{-20pt} 	
\end{figure}
% -------------- Vertical subfigures: -------------------------------
