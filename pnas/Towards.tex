...
%in the vanishing locus of a nonzero polynomial\footnote{Given a  nonzero polynomial $f$ in $T$ variables (real or complex), the vanishing locus of $f$ is the set 
%	$V(f) = \{(x_1, \ldots, x_T) \in \mathbb{R}^T (\text{resp. } \mathbb{C}^T) \vert f(x_1,\ldots x_T)=0\}$. By a general result in algebraic geometry $\dim V(f) < T$. As a consequence the complement of $V(f)$ in $\mathbb{R}^T$ (resp. $\mathbb{C}^T$) is dense.}. 
