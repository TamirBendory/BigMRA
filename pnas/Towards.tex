...
\end{equation*}
%
%We will consider the regime $N,M\to\infty$ and assume that $M_N=\Omega(N)$, so that $\gamma>0$. %In the sequel, we will suppress the explicit dependence of $M$ on $N$ for notational convenience.
...
\\ & + \underbrace{\frac{2}{N} \sum_{j=1}^{M} \sum_{i=0}^{L-1} x[i] \varepsilon[s_j + i + \ell]}_{\text{cross-term}}. 
%
\end{align*}
...
\end{equation}
Let $a_y^2:=a_y^2[0] + 2\sum_{j = 1}^{L-1}a_y^2[j]$ and recall from Proposition~\ref{prop:gamma}:
