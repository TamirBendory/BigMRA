...
\end{proposition}
\begin{proof}
In the limit, $(a^1_y)^2=\frac{\gamma^2}{L^2}\sum_{i=0}^{L-1}x[i]x[j]$. \TODO{In the previous equation, index $j$ is unaccounted for} Similarly,  $\sum_{\ell = 1}^{L-1}a_y^2[\ell]=\frac{\gamma}{L}\sum_{\ell = 1}^{L-1}\sum_{i=0}^{L-1-\ell}x[i]x[i+\ell]$ and $a_y^2[0]=\frac{\gamma}{L}\sum_{i=0}^{L-1}x^2[i] + \sigma^2$. The proof is concluded by noting that  $a_x^2[-\ell]=a_x^2[\ell]$. 
...
\begin{proposition} \label{prop:gamma_sigma}
	...
	The observed autocorrelations $a_y^1,a_y^2$ and  $a_y^3$ determine the ratio $\gamma$ and noise level $\sigma$ uniquely for a generic signal $x$. If $\gamma\geq \frac{1}{4}$, then this holds for any signal $x$ with nonzero mean. 
	\begin{proof}
...
\begin{corollary}
	...
	\begin{itemize}
		...
		\item   $x$ has nonzero mean  and $\gamma\geq\frac{1}{4}$.
	\end{itemize}
\end{corollary}
%
...
\begin{figure*}[t!]
	...
	\caption{\label{fig:cryo_detection} All micrographs are of size $7420^2$ pixels and the projections are taken from a volume of size $31^3$. The added noise was drawn from i.i.d.\ Gaussian distribution with zero mean and standard deviation $25$, corresponding to an SNR below $1/1024$.  
		The noise realization is identical in both micrographs. \TODO{Diagonal line}}
\end{figure*}
...
\begin{figure}[h]
	...
	\end{subfigure} 
	\caption{\label{fig:cryo_recon} Reconstructions from the first 3 autocorrelations.
		The ground truth volumes were expanded according to~\eqref{eq:volume_expansion_app} with cutoff $L$.
		The reconstruction is shown in gray while the (smoothed) ground truth in yellow. }
\end{figure}
...
Recall that $a_y^1 = \beta(\one^Tx)$ and $a_y^2[0] = \beta\|x\|^2+\sigma^2$, \TODO{the latter statements are only true in expectation, or almost surely in the limit though; should revise whole proof accordingly} where $\one\in\RL$ is the vector of all-ones. Taking the product:
...
The terms of $F_1$ can be also estimated from $a_y^3$, while taking the scaling and bias terms into account. This yields another observable:
\begin{align} 
...
\end{equation}
Let $E_3:=a_y^2[0] + 2\sum_{j = 1}^{L-1}a_y^2[j]$ and recall from Proposition~\ref{prop:gamma}:
...
E_5 := L \beta F_2 + (6L-3)\sigma^2a_y^1.
\end{equation*}
Taking the ratio:
\begin{equation*} 
...
\end{equation}	
For generic $x$,  this polynomial equation is not satisfied and the equations are independent. 
