function [m1, m2, m3] = moments_from_micrograph_steerable_matMult_GPU(I, L, batch_size)
...
m2 = zeros(sum(ang_freq == 0), 1, 'gpuArray');

blk_id = [];
for ii = 0:maxN
    blk_id(end+1:end+q_list(ii+1)^2, 1) = ii+1;
end
m3 = zeros(size(blk_id), 'gpuArray');
...
for t = 1:num_micros
    ...
    for batch = 1:batch_num
        ...
        batch_curr = zeros(2*L-1, 2*L-1, batch_size_actual);
        ...
        batch_curr = gpuArray(batch_curr);

%       Expand in PSWFs:
        coeff = Mt*batch_curr;
        m3_add = zeros(length(blk_id), 1, 'gpuArray');
        for N = 0:maxN
            if N == 0
